...
export default function useWalletStatus() {
    ...
    const walletContext = isConnected && stellarWallet.address
        ? "MULTI"
        : isConnected
            ? "EVM"
    ...
    const refreshEvmBalance = useCallback(async () => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : "Failed to load EVM balance";
            console.error("Failed to load EVM balance:", err);
            setEvmError(message);
            return false;
        }
    }, [evmAddress]);
    const refreshStellarBalance = useCallback(async (accountId) => {
